package com.riyosms.com.activities;
...
import com.riyosms.com.activities.SMSFilterActivity;
...
import com.riyosms.com.ApiManager;
import com.riyosms.com.AppConstants;
import com.riyosms.com.BuildConfig;
import com.riyosms.com.RiyoSmsUtils;
import com.riyosms.com.R;
import com.riyosms.com.dtos.RegisterDeviceInputDTO;
import com.riyosms.com.dtos.RegisterDeviceResponseDTO;
import com.riyosms.com.dtos.SimInfoCollectionDTO;
import com.riyosms.com.helpers.SharedPreferenceHelper;
import com.riyosms.com.helpers.VersionTracker;
import com.riyosms.com.helpers.HeartbeatManager;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (gatewayEnabled && stickyNotificationEnabled) {
            RiyoSmsUtils.startStickyNotificationService(mContext);
        ...
        String[] missingPermissions = Arrays.stream(AppConstants.requiredPermissions).filter(permission -> !RiyoSmsUtils.isPermissionGranted(mContext, permission)).toArray(String[]::new);
...
//        RiyoSmsUtils.startStickyNotificationService(mContext);
            ...
            apiCall.enqueue(new Callback<RegisterDeviceResponseDTO>() {
                ...
                public void onResponse(Call<RegisterDeviceResponseDTO> call, Response<RegisterDeviceResponseDTO> response) {
                    ...
                    if (enabled) {
                        ...
                        RiyoSmsUtils.stopStickyNotificationService(mContext);
                        // Cancel heartbeat
                ...
                public void onFailure(Call<RegisterDeviceResponseDTO> call, Throwable t) {
                    ...
                    RiyoSmsUtils.logException(t, "Error updating device");
                    compoundButton.setEnabled(true);
                }
            });
        });
        ...
        scanQRBtn.setOnClickListener(view -> {
            ...
            intentIntegrator.setPrompt("Go to riyosms.com/dashboard and click Register Device to generate QR Code");
        ...
        checkUpdatesBtn.setOnClickListener(view -> {
            ...
            String downloadUrl = "https://riyosms.com/download?currentVersion=" + encodedVersionInfo;
    ...
    private void renderAvailableSimOptions() {
        try {
            ...
            RiyoSmsUtils.getAvailableSimSlots(mContext).forEach(subscriptionInfo -> {
    ...
    public void onRequestPermissionsResult(int requestCode, String permissions[], int[] grantResults) {
        ...
        boolean allPermissionsGranted = Arrays.stream(permissions).allMatch(permission -> RiyoSmsUtils.isPermissionGranted(mContext, permission));
    ...
    private void handleRegisterDevice() {
        ...
        FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(task -> {
                    ...
                    simInfoCollection.setSims(RiyoSmsUtils.collectSimInfo(mContext));
                    ...
                    if (!deviceIdInput.isEmpty()) {
                        ...
                        apiCall.enqueue(new Callback<RegisterDeviceResponseDTO>() {
                            ...
                            public void onFailure(Call<RegisterDeviceResponseDTO> call, Throwable t) {
                                ...
                                Log.e(TAG, "API_ERROR "+ t.getMessage());
                                Log.e(TAG, "API_ERROR "+ t.getLocalizedMessage());
                                RiyoSmsUtils.logException(t, "Error registering device");
                                registerDeviceBtn.setEnabled(true);
                                registerDeviceBtn.setText("Update");
                            }
                        });
    ...
    private void handleRequestPermissions(View view) {
        boolean allPermissionsGranted = Arrays.stream(AppConstants.requiredPermissions).allMatch(permission -> RiyoSmsUtils.isPermissionGranted(mContext, permission));
        if (allPermissionsGranted) {
            Snackbar.make(view, "Already got permissions", Snackbar.LENGTH_SHORT).show();
            return;
        }
        String[] permissionsToRequest = Arrays.stream(AppConstants.requiredPermissions).filter(permission -> !RiyoSmsUtils.isPermissionGranted(mContext, permission)).toArray(String[]::new);
